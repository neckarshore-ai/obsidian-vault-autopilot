...
The trailing-dot enumeration regression (Steps 5–6) is locked by `tests/fixtures/windows-trailing-dot/` — folder named `030_Systems - reference material.` with two notes inside plus two notes outside, exercising both directions of the backlink-update path. Run `scripts/test-windows-trailing-dot.sh`. The Windows-side empirical verification (broken pattern fails, fixed pattern succeeds) is documented in the fixture's `README.md` — run on a Windows host before merging any change to enumeration in the four launch-scope skills.

The cross-platform clone-cluster preflight has its own test harness (`scripts/test-clone-preflight.sh`) and lives in [`clone-preflight.md`](clone-preflight.md), not in this file. It runs on every OS, so it is not a Windows-specific concern.
...
- **Clone-cluster birthtime patterns.** Cross-platform concern, handled by [`clone-preflight.md`](clone-preflight.md). That preflight runs unconditionally; this one runs only when `IS_WINDOWS=1`.

## Cost

On Windows: one `powershell.exe` invocation per skill run for Steps 2 + 5 (~100–300 ms combined). On macOS / Linux: one `uname -s` call (~1 ms) and the rest of this preflight is a no-op. The check is cheap enough to run every invocation.
