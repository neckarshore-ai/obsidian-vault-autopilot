...
>
> Affected users: any v0.1.4 install on a non-UTC macOS host that ran `property-enrich` (or another date-derivation skill) on a cloned vault between v0.1.4 ship (2026-05-07) and v0.1.5 upgrade. Re-run `property-enrich` after upgrading to v0.1.5 — the SKIP-gate will now fire correctly and Class-C findings will identify the cluster files; the user decides what to do with previously-poisoned `created` values.
>
